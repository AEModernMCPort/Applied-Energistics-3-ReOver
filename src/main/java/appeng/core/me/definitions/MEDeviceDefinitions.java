...
public class MEDeviceDefinitions<N extends NetDevice<N, P>, P extends PhysicalDevice<N, P>> extends Definitions<DeviceRegistryEntry<N, P>, IDeviceDefinition<N, P>> implements IMEDeviceDefinitions<N, P> {
	...
	public MEDeviceDefinitions(DefinitionFactory registry){
		...
		this.simplyPanel = registry.<DeviceRegistryEntry<ImportBus.Network, ImportBus.Physical>, IDeviceDefinition<ImportBus.Network, ImportBus.Physical>, IDeviceBuilder<ImportBus.Network, ImportBus.Physical, ?>, Void>definitionBuilder(new ResourceLocation(AppEng.MODID, "panel"), ih()).deserializer(re -> new NetDeviceBaseLoader<>((uuid, netBlock) -> new ImportBus.Network(re, uuid, netBlock))).build();
	}
