...
public class MEDeviceDefinitions<N extends NetDevice<N, P>, P extends PhysicalDevice<N, P>> extends Definitions<DeviceRegistryEntry<N, P>, IDeviceDefinition<N, P>> implements IMEDeviceDefinitions<N, P> {
	...
	public MEDeviceDefinitions(DefinitionFactory registry){
		...
		this.exportBus = registry.<DeviceRegistryEntry<ExportBus.Network, ExportBus.Physical>, IDeviceDefinition<ExportBus.Network, ExportBus.Physical>, IDeviceBuilder<ExportBus.Network, ExportBus.Physical, ?>, Void>definitionBuilder(new ResourceLocation(AppEng.MODID, "export_bus"), ih()).deserializer(re -> new NetDeviceBaseLoader<>((uuid, netBlock) -> new ExportBus.Network(re, uuid, netBlock))).build();
