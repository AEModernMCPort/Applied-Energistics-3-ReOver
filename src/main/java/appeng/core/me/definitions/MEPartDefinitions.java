...
public class MEPartDefinitions<P extends Part<P, S>, S extends Part.State<P, S>> extends Definitions<P, IPartDefinition<P, S>> implements IMEPartDefinitions<P, S> {
	...
	public MEPartDefinitions(DefinitionFactory registry){
		...
		importBus = registry.<P, IPartDefinition<P, S>, IPartBuilder<P, S, ?>, P>definitionBuilder(new ResourceLocation(AppEng.MODID, "import_bus"), ih(new ImportBusImpl.Part())).rootMesh(new ResourceLocation(AppEng.MODID, "device/import_bus/import_bus.obj")).createDefaultPlacerItem().build();
		exportBus = registry.<P, IPartDefinition<P, S>, IPartBuilder<P, S, ?>, P>definitionBuilder(new ResourceLocation(AppEng.MODID, "export_bus"), ih(new ExportBusImpl.Part())).rootMesh(new ResourceLocation(AppEng.MODID, "device/export_bus/export_bus.obj")).createDefaultPlacerItem().build();
