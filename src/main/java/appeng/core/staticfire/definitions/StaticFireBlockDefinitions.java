...
public class StaticFireBlockDefinitions extends Definitions<Block, IBlockDefinition<Block>> implements IStaticFireBlockDefinitions{
    ...
    public StaticFireBlockDefinitions(DefinitionFactory registry){
        ...
        registry.definitionBuilder(new ResourceLocation(AppEng.MODID, "sky_stone", ih(item)).<ItemBlock, BlockItemCustomizer.UseDefaultItemCustomize>createItem(builder -> builder.<IDefinitionBuilder.DefinitionInitializationComponent.PreInit<ItemBlock, IItemDefinition<ItemBlock>>>initializationComponent(Side.CLIENT, def -> new ItemMeshDefinitionComponent.BlockStateMapper2ItemMeshDefinition.createByMetadata(Block.getBlockFromItem(def.maybe().get())))));
