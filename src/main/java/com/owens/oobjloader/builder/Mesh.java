...
public class Mesh implements IMesh {
	...
	private Logger log = LogManager.getLogger(Mesh.class.getName());
	...
	public void setObjResourceLocation(ResourceLocation location){
	...
	public void addFace(int[] vertexIndices){
		...
		while(loopi < vertexIndices.length){
			...
			if(((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())){
				...
				log.log(FATAL, "Index for geometric vertex=" + vertexIndex + " is out of the current range of geometric vertex values 1 to " + verticesG.size() + ", ignoring");
			}
			...
			if(vertexIndex != EMPTY_VERTEX_VALUE){
				...
				if(((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())){
					...
					log.log(FATAL, "Index for texture vertex=" + vertexIndex + " is out of the current range of texture vertex values 1 to " + verticesT.size() + ", ignoring");
				}
			}
			...
			if(vertexIndex != EMPTY_VERTEX_VALUE){
				...
				if(((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())){
					...
					log.log(FATAL, "Index for vertex normal=" + vertexIndex + " is out of the current range of vertex normal values 1 to " + verticesN.size() + ", ignoring");
				}
			}

			if(fv.v == null){
				log.log(FATAL, "Can't add vertex to face with missing vertex!  Throwing away face.");
				faceErrorCount++;
				return;
			}
	...
	// @TODO: I think I need to just delete this... because we now parse material lib files in OObjMeshLoader.java in processMaterialLib()
	...
	public void setMapDecalDispBump(int type, ResourceLocation location){
		if(type == MTL_MAP_KA){
			currentMaterialBeingParsed.mapKaLocation = location;
		} else if(type == MTL_MAP_KD){
			currentMaterialBeingParsed.mapKdLocation = location;
		} else if(type == MTL_MAP_KS){
			currentMaterialBeingParsed.mapKsLocation = location;
		} else if(type == MTL_MAP_NS){
			currentMaterialBeingParsed.mapNsLocation = location;
		} else if(type == MTL_MAP_D){
			currentMaterialBeingParsed.mapDLocation = location;
		} else if(type == MTL_DECAL){
			currentMaterialBeingParsed.decalLocation = location;
		} else if(type == MTL_DISP){
			currentMaterialBeingParsed.dispLocation = location;
		} else if(type == MTL_BUMP){
			currentMaterialBeingParsed.bumpLocation = location;
		}
	}

	public void setRefl(int type, ResourceLocation location){
		currentMaterialBeingParsed.reflType = type;
		currentMaterialBeingParsed.reflLocation = location;
	}
	...
	public void doneParsingMaterial(){
		...
		// better than quiet bugs.)   This method ( doneParsingMaterial() ) is called by OObjMeshLoader when
		// it finished parsing a .mtl file.
		//
		// @TODO: We can make this not throw an exception if we simply add a check for a null
		// currentMaterialBeingParsed at the start of each material setter method in Mesh... but that
	...
	public void doneParsingObj(ResourceLocation location){
		log.log(INFO, "Loaded location '" + location + "' with " + verticesG.size() + " verticesG, " + verticesT.size() + " verticesT, " + verticesN.size() + " verticesN and " + faces.size() + " faces, of which " + faceTriCount + " triangles, " + faceQuadCount + " quads, and " + facePolyCount + " with more than 4 points, and faces with errors " + faceErrorCount);
	}
}
