...
public class OObjMeshLoader<M extends IMesh> {
	...
	private Logger log = LogManager.getLogger(OObjMeshLoader.class.getName());

	public M builder = null;
	ResourceHelper resourceManager;
	ResourceLocation location;

	public OObjMeshLoader(M builder, ResourceHelper resourceManager, ResourceLocation location)
			throws IOException{
		...
		builder.setObjResourceLocation(location);
		parseObjFile();
		builder.doneParsingObj(location);
	}

	private void parseObjFile() throws IOException{
		int lineCount = 0;
		BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(resourceManager.getInputStream(location)));
		...
		while(true){
			...
			if(line.startsWith("#")) // comment
			{
				...
				log.log(WARN, "line " + lineCount + " unknown line |" + line + "|");
			}
			lineCount++;
		}
	...
	private void processMaterialLib(String line) throws IOException{
		...
		if(null != matlibnames){
			for(int loopi = 0; loopi < matlibnames.length; loopi++){
				try{
					parseMtlFile(ResourceLocationHelper.relative(location, matlibnames[loopi]));
				} catch(FileNotFoundException e){
					log.log(FATAL, "Can't find material file name='" + matlibnames[loopi] + "', e=" + e);
				}
			}
		}
	}
	...
	// ----------------------------------------------------------------------
	private void parseMtlFile(ResourceLocation mtlLocation) throws FileNotFoundException, IOException{
		int lineCount = 0;
		BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(resourceManager.getInputStream(mtlLocation)));
		...
		log.log(INFO, "OObjMeshLoader.parseMtlFile: Loaded " + lineCount + " lines");
	}
	...
	private void processReflectivityTransmissivity(String fieldName, String line){
		...
		if(null == tokens){
			log.log(FATAL, "Got Ka line with no tokens, line = |" + line + "|");
			return;
		}
		if(tokens.length <= 0){
			log.log(FATAL, "Got Ka line with no tokens, line = |" + line + "|");
		...
		if(tokens[0].equals("spectral")){
			...
			log.log(WARN, "Sorry Charlie, this parse doesn't handle \'spectral\' parsing.  (Mostly because I can't find any info on the spectra.rfl file.)");
			return;
			// 	    if(tokens.length < 2) {
			// 		log.log(FATAL, "Got spectral line with not enough tokens, need at least one token for spectral file and one value for factor, found "+(tokens.length-1)+" line = |"+line+"|");
			// 		return;
			// 	    }
			...
			if(tokens.length < 2){
				log.log(FATAL, "Got xyz line with not enough x/y/z tokens, need at least one value for x, found " + (tokens.length - 1) + " line = |" + line + "|");
				return;
			}
	...
	private void processIllum(String line){
		...
		if((illumModel < 0) || (illumModel > 10)){
			log.log(FATAL, "Got illum mesh value out of range (0 to 10 inclusive is allowed), value=" + illumModel + ", line=" + line);
			return;
		}
	...
	private void processMapDecalDispBump(String fieldname, String line){
		int type = IMesh.MTL_MAP_KA;
		if(fieldname.equals(MTL_MAP_KD)){
			type = IMesh.MTL_MAP_KD;
		} else if(fieldname.equals(MTL_MAP_KS)){
			type = IMesh.MTL_MAP_KS;
		} else if(fieldname.equals(MTL_MAP_NS)){
			type = IMesh.MTL_MAP_NS;
		} else if(fieldname.equals(MTL_MAP_D)){
			type = IMesh.MTL_MAP_D;
		} else if(fieldname.equals(MTL_DISP)){
			type = IMesh.MTL_DISP;
		} else if(fieldname.equals(MTL_DECAL)){
			type = IMesh.MTL_DECAL;
		} else if(fieldname.equals(MTL_BUMP)){
			type = IMesh.MTL_BUMP;
		}

		ResourceLocation resourceLocation = new ResourceLocation(line.substring(fieldname.length()).trim());
		builder.setMapDecalDispBump(type, resourceLocation);
	...
	private void processRefl(String line){
		...
		int type = IMesh.MTL_REFL_TYPE_UNKNOWN;
		line = line.substring(MTL_REFL.length()).trim();
		if(line.startsWith("-type")){
			line = line.substring("-type".length()).trim();
			if(line.startsWith(MTL_REFL_TYPE_SPHERE)){
				type = IMesh.MTL_REFL_TYPE_SPHERE;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_SPHERE.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_TOP)){
				type = IMesh.MTL_REFL_TYPE_CUBE_TOP;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_TOP.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_BOTTOM)){
				type = IMesh.MTL_REFL_TYPE_CUBE_BOTTOM;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_BOTTOM.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_FRONT)){
				type = IMesh.MTL_REFL_TYPE_CUBE_FRONT;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_FRONT.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_BACK)){
				type = IMesh.MTL_REFL_TYPE_CUBE_BACK;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_BACK.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_LEFT)){
				type = IMesh.MTL_REFL_TYPE_CUBE_LEFT;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_LEFT.length()).trim());
			} else if(line.startsWith(MTL_REFL_TYPE_CUBE_RIGHT)){
				type = IMesh.MTL_REFL_TYPE_CUBE_RIGHT;
				resourceLocation = new ResourceLocation(line.substring(MTL_REFL_TYPE_CUBE_RIGHT.length()).trim());
			} else {
				log.log(FATAL, "unknown material refl -type, line = |" + line + "|");
				return;
			}
		} else {
			resourceLocation = new ResourceLocation(line);
		}

		builder.setRefl(type, resourceLocation);
	}
}
